...
class ChatStreamManager:
        ...
        if self.checkpoint_saver:
            if self.db_uri.startswith("postgresql://") or self.db_uri.startswith(
                ...
                logger.warning(
                    ...
                    "Supported schemes: postgresql://, postgres://"
                )
        else:
            logger.warning("Checkpoint saver is disabled")
    ...
    def _create_chat_streams_table(self) -> None:
        ...
        try:
            with self.postgres_conn.cursor() as cursor:
                ...
                logger.info("Chat streams table created/verified successfully")
        except Exception as e:
            logger.error(f"Failed to create chat_streams table: {e}")
    ...
    def process_stream_message(
        ...
        temporarily in memory and permanently in DB when complete.
        ...
        if not thread_id or not isinstance(thread_id, str):
            logger.warning("Invalid thread_id provided")
            return False
        ...
        try:
            ...
            logger.error(f"Error processing stream message for thread {thread_id}: {e}")
            return False
    ...
    def _persist_complete_conversation(
        ...
        """
        Persist completed conversation to database (PostgreSQL).
        ...
        try:
            ...
            logger.error(f"Error persisting conversation for thread {thread_id}: {e}")
            return False
...
_default_manager = ChatStreamManager(
    ...
    db_uri=get_str_env("LANGGRAPH_CHECKPOINT_DB_URL", None),
)
