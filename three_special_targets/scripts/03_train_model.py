...
def train_single_target(target_id, target_name, ref_params):
    ...
    # 加载测试数据用于最终评估
    X_test, y_test = load_test_data(target_id)
    test_df = None
    test_path = output_dir / target_id / "test_weekly.csv"
    if test_path.exists():
    ...
    train_mda = mean_directional_accuracy(y_train.values, y_pred_train)
    print(f"    训练集MDA: {train_mda:.4f}")
...
def main():
    ...
    if train_records:
        ...
        for tid, rec in train_records.items():
            ...
            train_mda = rec.get("train_mda", "N/A")
            train_mda_str = f"{train_mda:.4f}" if train_mda is not None else "N/A"
            test_mda = rec.get("test_mda", "N/A")
            test_mda_str = f"{test_mda:.4f}" if test_mda is not None else "N/A"
            test_rmse = rec.get("test_rmse", "N/A")
            test_rmse_str = f"{test_rmse:.4f}" if test_rmse is not None else "N/A"
            print(f"  {tid:<12s} {tname:<40s} trainMDA={train_mda_str} testMDA={test_mda_str} testRMSE={test_rmse_str}")
